...
<x-layouts.app :title="$title">
    <div class="max-w-4xl mx-auto p-6 md:p-10">
        ...
        @if($errors->any())
            <div class="mb-4 p-3 rounded bg-red-50 text-red-700" role="alert" aria-live="polite">{{ $errors->first() }}</div>
        @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    @foreach($groups as $g)
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="groups[]" value="{{ $g->id }}" aria-label="Selecionar grupo {{ $g->name }}">
                            <span>{{ $g->name }}</span>
                        </label>
                    @endforeach
                </div>
