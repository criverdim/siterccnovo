...
class PasswordController extends Controller
{
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        if (! $user) {
            return back()->with('status', 'Se existir, enviaremos o link de redefinição');
        }
    ...
    public function reset(Request $request)
    {
        ...
        if (! $record) {
            return back()->withErrors(['email' => 'Token inválido'])->withInput();
        }
        if (now()->diffInMinutes($record->created_at) > 60 * 24) {
            return back()->withErrors(['email' => 'Token expirado'])->withInput();
        }
